...
import '../../../Defines'
import '../../../Settings' as Preferences
...
Item {
    ...
    */
    ...
    Preferences.CustomWaveform { id: customWF }
    Preferences.DeckHeader { id: customHeader }
...
//------------------------------------------------------------------------------------------------------------------
    ...
    MappingProperty { id: autoImport; path: "mapping.settings.autoimport" }
    MappingProperty { id: autoExport; path: "mapping.settings.autoexport" }
...
//------------------------------------------------------------------------------------------------------------------
    ...
    property bool shift: (globalShift.value && globalShiftEnabled.value) || (sideShift.value && !globalShiftEnabled.value) || false
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    MappingProperty { id: decks; path: "mapping.settings.decks_assignment" }
    function getTopDeckId() {
        if (screen.flavor != ScreenFlavor.D2) return isLeftScreen ? 1 : 2
        switch (decks.value) {
            ...
            default: return isLeftScreen ? 1 : 2;
        }
    }
    function getBottomDeckId() {
        if (screen.flavor != ScreenFlavor.D2) return isLeftScreen ? 3 : 4
        switch (decks.value) {
            ...
            default: return isLeftScreen ? 3 : 4;
        }
    }
    ...
    Side {
        id: side
        topDeckId: getTopDeckId()
        bottomDeckId: getBottomDeckId()
...
//--------------------------------------------------------------------------------------------------------------------
    ...
    MappingProperty { id: editMode; path: propertiesPath + ".beatgrid.edit"; onValueChanged: { showSideOverlays() } }
