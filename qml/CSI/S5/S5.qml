...
//------------------------------------------------------------------------------------------------------------------
    ...
    onRunningChanged: {
        console.log("S5:", running ? "Running" : "Not running")
        if (running) {
            left.initializeModule();
            right.initializeModule();

            settingsManager.readTraktorSettings();
            settingsManager.assignModSettings()
        }
    }

    onMappingLoaded: {
        console.log("S5: Mapping loaded!");
    }
